"""
IPPO Policy implementation for discrete action spaces.

Adapted from the reference IPPO implementation to work with
...
from IPPO.networks.utils import normalize_advantages
from IPPO.utils.reward_normalizer import RewardNormalizer


class IPPOPolicy(nn.Module):
    """
    Independent PPO Policy for discrete action spaces.
    
    This implements one decentralized PPO learner. Each agent owns its actor,
    local critic, and optimizers.
    
    Args:
        actor: Actor network
        critic: Local critic network for this agent
    ...
    def set_critic(self, critic: LocalCritic):
        """Set the local critic for this policy."""
        self.critic = critic
    ...
    def process_fn(
        ...
        with torch.no_grad():
            values = self.critic(obs).squeeze(-1).cpu().numpy()
            next_values = self.critic(obs_next).squeeze(-1).cpu().numpy()
